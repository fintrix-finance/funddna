...
async function buildAnalysis(fund, { max = 10, grounded = true } = {}) {
  ...
  if (!imp.length) {
    const top = fund.holdings.filter(h => h.weight > 0).slice(0, max);
    if (!top.length) return { code:fund.code, error:'No disclosed holdings', holdings:[], portfolio:null };
    const S = [{ id:'S1', kind:'holdings', title:`Portfolio holdings as of ${fund.portfolioDate}`, ...fund.sources.holdings }];
    let out = null, err = null;
    try { out = await G.reasonAssets({ fund:{ name:fund.name, category:fund.category,
      portfolio_date:fund.portfolioDate, holdings:top.map(h => ({ name:h.name, weight:h.weight, type:h.type, instrument:h.instrument })) }, sources:[{ id:'S1', kind:'holdings', date:fund.portfolioDate }] }); }
    catch (e) { err = String(e.message || e); }
    if (out) {
      const allowed = new Map(top.map(h => [h.name.toLowerCase(), h]));
      out.holdings = (out.holdings || []).filter(h => allowed.has(String(h.name).toLowerCase())).map(h => {
        const src = allowed.get(h.name.toLowerCase());
        return { name:src.name, weight:src.weight, sector:src.type,
          what_changed:[], expected:[], seeing:[], why_it_matters:String(h.why_it_matters||''), watch:[] };
      }).filter(h => h.why_it_matters);
      if (out.portfolio) {
        out.portfolio.drivers = (out.portfolio.drivers || []).filter(x => String(x.text||'').trim()).map(x => ({text:String(x.text),src:['S1']}));
        out.portfolio.priced_in=[]; out.portfolio.themes=[]; out.portfolio.macro=[]; out.portfolio.could_change=[];
      }
    }
    return { code:fund.code, model:G.MODEL(), grounded:false, error:err,
      holdings:out?.holdings||[], portfolio:out?.portfolio||null, sources:S, generatedAt:new Date().toISOString(), assetOnly:true };
  }
