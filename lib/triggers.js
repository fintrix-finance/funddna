...
const holdings = f => f.holdings.filter(h => h.type === 'EQUITY').slice(0, 10).map(h => ({ name: h.name, weight: h.weight }));
...
function compare(current, baseline, analysis) {
  ...
  if (current.portfolioDate && oldDate && current.portfolioDate > oldDate) {
    ...
    if (current.holdings.some(h => !prev.has(h.name.toLowerCase()) || Math.abs(h.weight - prev.get(h.name.toLowerCase())) >= .5) || old.some(h => !current.holdings.some(n => n.name.toLowerCase() === h.name.toLowerCase()))) reasons.push('top holding changed by 0.5 percentage points or entered/left');
  }
  // Compare with NAV at the last successful signal scan, not intraday quotes.
  if (baseline && baseline.nav > 0 && current.nav > 0 && Math.abs(current.nav / baseline.nav - 1) >= .05) reasons.push('NAV moved at least 5% since analysis');
