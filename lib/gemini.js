  ...
  try {
    ...
    for (let a = 0; a < 3; a++) {
      ...
      if (r.status === 429 || r.status === 503) {
        last = `Gemini ${r.status}: ${(j.error && j.error.message || '').slice(0, 300)}`;
        const hint = JSON.stringify(j).match(/"retryDelay":"(\d+)s"/); const wait = Math.min(hint ? +hint[1] * 1000 : 4000 * (a + 1), 30000);
        await new Promise(s => setTimeout(s, wait)); continue;
      }
      if (!r.ok) throw new Error(`Gemini ${r.status}: ${(j.error && j.error.message || '').slice(0, 300)}`);
      return j;
    }
    throw new Error(last || 'Gemini rate-limited');
...
// Non-equity funds have fund, bond, cash or real-asset positions rather than listed
// companies. Do not manufacture earnings, forecasts, credit ratings or catalysts.
async function reasonAssets(pack) {
  const sys = `You are FundDNA's reasoning layer. Explain the disclosed holdings of a non-equity Indian mutual fund. Use ONLY the supplied dated holdings, weights, instrument types, category and sources. Do not infer a bond's rating, duration, yield, issuer solvency, ETF physical backing or forecast from its name. Do not say you reviewed filings or earnings. Cite source S1 for factual holding claims. Return ONLY JSON:
{"holdings":[{"name":str,"weight":number,"sector":str,"what_changed":[],"expected":[],"seeing":[],"why_it_matters":str,"watch":[]}],"portfolio":{"statement":str,"interp":[str,str],"line":str,"drivers":[{"text":str,"src":["S1"]}],"themes":[],"macro":[],"priced_in":[],"could_change":[],"risk_note":str}}.
Each holding's why_it_matters should explain concentration or type exposure, not pretend to know the issuer's prospects. Portfolio statement and line must be concise and grounded in the data. No recommendation.`;
  const j = await call({ systemInstruction: { parts: [{ text: sys }] }, contents: [{ role:'user', parts:[{ text: JSON.stringify(pack) }] }], generationConfig:{ temperature:0.1, responseMimeType:'application/json' } }, 180000);
  const txt = (j.candidates?.[0]?.content?.parts || []).map(p => p.text || '').join('');
  return JSON.parse(txt.replace(/^```json\s*|```$/g,''));
}
module.exports.reasonAssets = reasonAssets;
